    ...
    for (i, row) in range.rows().enumerate() {
        let string_row: Vec<String> = row
            ...
            .map(|cell| match cell {
                ...
                Data::Float(f) => {
                    // Avoid scientific notation if it's an integer
                    if f.fract() == 0.0 {
                        format!("{:.0}", f)
                    } else {
                        f.to_string()
...
static RE_ALPHA_NUM: Lazy<Regex> = Lazy::new(|| Regex::new(r"([A-Z]+) (\d+)").unwrap());
static RE_DOT_SPACE: Lazy<Regex> = Lazy::new(|| Regex::new(r"(\d) \.").unwrap());
